...
const PokemonService = {
    ...
    fetchPokemon: async (url: string) => {
        ...
        const pokemonInfo: PokemonType = {
            ...
            height: data.height,
            photoUrl,
            stats: data.stats.map(s => ({baseStat: s.base_stat, effort: s.effort, name: s.stat.name, url: s.stat.url})),
            types: data.types.map(t => ({name: inferTypeFromString(t.type.name), slot: t.slot, url: t.type.url})),
            weight: data.weight
        }
        return pokemonInfo
    },
